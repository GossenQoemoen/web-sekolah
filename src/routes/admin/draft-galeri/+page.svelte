<script lang="ts">
	...
	type Album = { id: string; judul: string; keterangan: string; unit: string; created_at: string; profiles: { nama: string } | null; galeri: { id: string }[] };

	let list = $state<Album[]>([]);
	let msg = $state('');

	async function load() {
		const { data, error } = await supabase
			.from('album')
			.select('id, judul, keterangan, unit, created_at, profiles!album_author_profiles_fkey(nama), galeri(id)')
			.eq('status', 'draft')
	...
	async function publish(id: string) {
		const { error } = await supabase.from('album').update({ status: 'published' }).eq('id', id);
		if (error) msg = 'Error: ' + error.message;
		else await load();
	}
...
<div class="page-head">
	<h1 class="page-title">🖼️ Draft Galeri</h1>
	<p class="page-sub">Album yang menunggu persetujuan untuk dipublikasikan.</p>
</div>
...
<div class="card">
	{#if list.length === 0}
		<p class="empty">Tidak ada draft album.</p>
	{:else}
		<div class="list">
			{#each list as a}
				<div class="row">
					<div class="info">
						...
						<div class="meta">
							{#if a.profiles?.nama}<span class="author">👤 {a.profiles.nama}</span>{/if}
							{#if a.unit}<span class="unit-tag">{a.unit}</span>{/if}
							<span class="foto-count">📷 {a.galeri?.length ?? 0} foto</span>
							<span class="tgl">{new Date(a.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
						</div>
						{#if a.keterangan}<p class="ket">{a.keterangan}</p>{/if}
					</div>
					<div class="actions">
						<button onclick={() => publish(a.id)} class="btn-publish">✓ Publish</button>
						<button onclick={() => hapus(a.id)} class="btn-hapus">Hapus</button>
					</div>
				</div>
			{/each}
		</div>
...
<style>
	...
	.ket { margin: 0.3rem 0 0; font-size: 0.8rem; color: var(--muted); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
	...
	.foto-count { font-size: 0.72rem; color: var(--muted); }
